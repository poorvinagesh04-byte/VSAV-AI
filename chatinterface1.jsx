...
// LINE 2: UPDATED IMPORTS to include the new CRUD functions from firebase.js
import { auth, db, insertStudent, updateStudent, deleteStudent } from './firebase'; 
...
import {
    ...
    doc,
    updateDoc,
    deleteDoc // Added deleteDoc import, just in case you need it here later
...
const ChatInterface = () => {
    ...
    // LINE 430: START OF NEW STATE FOR CRUD OPERATIONS
    const [studentIdToUpdate, setStudentIdToUpdate] = useState('');
    const [newGpa, setNewGpa] = useState('');
    const [studentIdToDelete, setStudentIdToDelete] = useState('');
    // END OF NEW STATE FOR CRUD OPERATIONS
    ...
    // --- Helper: Create Session (Existing Function) ---
    ...
    // --- Helper: Save Message (Existing Function) ---
    ...
    // LINE 530: START OF NEW CRUD HANDLER FUNCTIONS

    // --- Data Operation Handlers (CRUD) ---

    // --- 1. INSERT/CREATE Handler ---
    const handleInsertStudent = async () => {
        // Simple, predefined record for demonstration. 
        // In a real app, this data would come from dedicated input fields.
        const name = "New Student Example"; 
        const gpa = 3.0;
        const major = "Computer Science";
        
        if (!userId) {
            setError("User must be authenticated to perform database operations.");
            return;
        }
        setIsLoading(true);
        setError(null);
        ...
        try {
            // Calls the function defined and exported in firebase.js
            const newId = await insertStudent(newRecord); 
            await saveMessage('ai', { nlSummary: `✅ Inserted new student **${name}** into Firestore. New ID: **${newId}**` });
        } catch (error) {
    ...
    const handleUpdateStudent = async () => {
        if (!studentIdToUpdate || !newGpa) {
            setError("ID and new GPA are required for update.");
            return;
        }
        if (!userId) {
            setError("User must be authenticated to perform database operations.");
        ...
        try {
            ...
            // Calls the function defined and exported in firebase.js
            await updateStudent(studentIdToUpdate, updatedFields); 
            await saveMessage('ai', { nlSummary: `✅ Successfully updated student **${studentIdToUpdate}**. New GPA: **${newGpa}**` });
        } catch (error) {
            setError(`Update failed: ${error.message}. Ensure the ID is correct.`);
        } finally {
            setIsLoading(false);
            setStudentIdToUpdate(''); // Clear inputs after success
            setNewGpa('');
        }
    };
    ...
    const handleDeleteStudent = async () => {
        if (!studentIdToDelete) {
            setError("Student ID is required for deletion.");
            return;
        }
        if (!userId) {
            setError("User must be authenticated to perform database operations.");
        ...
        if (!window.confirm(`Are you sure you want to permanently delete student ${studentIdToDelete}?`)) return;
        ...
        try {
            ...
            await saveMessage('ai', { nlSummary: `🗑️ Successfully deleted student with ID: **${studentIdToDelete}**` });
        } catch (error) {
            setError(`Deletion failed: ${error.message}. Ensure the ID is correct.`);
        } finally {
            setIsLoading(false);
            setStudentIdToDelete(''); // Clear input after success
        }
    };

    // LINE 590: END OF NEW CRUD HANDLER FUNCTIONS

    // ... existing functions (callGeminiApi, handleSpeechInput, etc.)

    const callGeminiApi = async (payload, retries = 3) => {
    // ... rest of callGeminiApi remains the same
    
    // ...
    
    // --- Render (Starting around Line 826) ---
    return (
        <div className="flex h-screen bg-[#0f172a] font-sans overflow-hidden text-slate-200 selection:bg-indigo-500/30">
            ...
            <div className="hidden md:flex w-72 bg-[#0f172a] border-r border-slate-800 flex-col z-10 relative">
                ...
                <div className="flex-1 overflow-y-auto p-4">
                    ...
                    {/* LINE 630: START OF NEW CRUD UI ELEMENTS */}
                    <hr className="border-slate-800 my-4" />
                    <h3 className="text-[10px] font-bold text-indigo-400 uppercase tracking-widest mb-3 ml-1 flex items-center">
                        <DatabaseIcon className="w-3 h-3 mr-1" /> Database Actions
                    </h3>

                    <div className="space-y-4">
                        {/* 1. INSERT Button */}
                        <div className="p-3 bg-slate-800/50 rounded-xl border border-slate-700/50">
                            <h4 className="text-[10px] font-bold text-slate-400 mb-2 uppercase">1. Insert New Student</h4>
                            <button onClick={handleInsertStudent} className="w-full text-xs bg-emerald-600/30 hover:bg-emerald-600/50 text-emerald-300 py-1.5 rounded transition-colors" disabled={isLoading || !userId}>
                                Add Example
                            </button>
                            <p className="text-[9px] text-slate-500 mt-2">Adds a simplified record to the Firestore collection.</p>
                        </div>

                        {/* 2. UPDATE Form */}
                        <div className="p-3 bg-slate-800/50 rounded-xl border border-slate-700/50">
                            <h4 className="text-[10px] font-bold text-slate-400 mb-2 uppercase">2. Update GPA (by ID)</h4>
                            <input 
                                className="w-full text-xs p-2 mb-2 bg-slate-900 border border-slate-700 rounded placeholder-slate-500" 
                                placeholder="Student Firestore ID" 
                                value={studentIdToUpdate} 
                                onChange={e => setStudentIdToUpdate(e.target.value)} 
                                disabled={isLoading || !userId} 
                            />
                            <input 
                                className="w-full text-xs p-2 mb-2 bg-slate-900 border border-slate-700 rounded placeholder-slate-500" 
                                placeholder="New GPA (e.g., 3.9)" 
                                type="number" 
                                step="0.1"
                                value={newGpa} 
                                ...
                                disabled={isLoading || !userId} 
                            />
                            <button onClick={handleUpdateStudent} className="w-full text-xs bg-yellow-600/30 hover:bg-yellow-600/50 text-yellow-300 py-1.5 rounded transition-colors" disabled={isLoading || !studentIdToUpdate || !newGpa || !userId}>
                                Update Record
                            </button>
                        </div>
                        ...
                        {/* 3. DELETE Form */}
                        <div className="p-3 bg-slate-800/50 rounded-xl border border-slate-700/50">
                            <h4 className="text-[10px] font-bold text-slate-400 mb-2 uppercase">3. Delete Student (by ID)</h4>
                            <input 
                                className="w-full text-xs p-2 mb-2 bg-slate-900 border border-slate-700 rounded placeholder-slate-500" 
                                placeholder="Student Firestore ID" 
                                ...
                                disabled={isLoading || !userId} 
                            />
                            <button onClick={handleDeleteStudent} className="w-full text-xs bg-red-600/30 hover:bg-red-600/50 text-red-300 py-1.5 rounded transition-colors" disabled={isLoading || !studentIdToDelete || !userId}>
                                Delete Record
                            </button>
                        </div>
                    </div>
                    <hr className="border-slate-800 my-4" />
                    {/* END OF NEW CRUD UI ELEMENTS */}
            ...
            {/* Main Chat Area */}
            {/* ... remaining JSX (Main Chat Area, Input Area) remains the same ... */}
